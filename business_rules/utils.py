...
def export_rule_data(variables, actions, chinese=False):
    ...
    actions_data = actions.get_all_actions(chinese)
    variables_data = variables.get_all_variables(chinese)
    ...
    for variable_class in inspect.getmembers(operators, lambda x: getattr(x, 'export_in_rule_data', False)):
        ...
        variable_type_operators[variable_type.name] = variable_type.get_all_operators(chinese)
